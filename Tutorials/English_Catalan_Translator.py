...
embedding_dim = 512
units = 1024
vocab_inp_size = len(input_lang.word_to_id)
vocab_tar_size = len(target_lang.word_to_id)
...
def gru(units):
    ...
    if tf.test.is_gpu_available():
        return keras.layers.CuDNNGRU(units,                 # CuDNNGRU requires different setup
...
class Decoder(tf.keras.Model):
    ...
    def call(self, x, hidden, enc_output):
        ...
        # layer below is added to calculate the score for use in attention
...
for epoch in range(EPOCHS):
    ...
    for (batch, (inp, targ)) in enumerate(dataset):
        ...
        with tf.GradientTape() as tape:
            ...
            dec_input = tf.expand_dims([target_lang.word_to_id['<start>']] * BATCH_SIZE, 1)
...
def translate(sentence, encoder, decoder, inp_lang, targ_lang, max_length_inp,
              max_length_targ):
...
##translate('I am well', encoder, decoder, input_lang, target_lang,
##          max_input_length, max_target_length)

translate('ich bin gut', encoder, decoder, input_lang, target_lang,
          max_input_length, max_target_length)
